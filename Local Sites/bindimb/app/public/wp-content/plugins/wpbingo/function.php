<?php
	...
	function wpbingo_posted_on() {
		...
		printf( '<div class="entry-dates"><time class="entry-date" datetime="%2$s"><span>%3$s</span><span>%4$s</span></time></div>',
	...
	function wpbingo_posted_on2() {
		// Set up and print post meta information.
		printf( '<span class="entry-date"><time class="entry-date" datetime="%6$s">%3$s %4$s, %5$s</time></span>',
			esc_url( get_permalink() ),
			esc_attr( get_the_date( 'l' ) ),
						...
						foreach($attributes as $key => $attribute){
							...
							if($term){
								if(isset($tax_attribute->attribute_type) && $tax_attribute->attribute_type == "color"){
									...
									if($image && !in_array($color, $colors)){
									...
									$image = wp_get_attachment_image_src( $variation['image_id'], 'woocommerce_thumbnail');
									if($image && !in_array($term->name, $labels)){
									...
									$image = wp_get_attachment_image_src( $variation['image_id'], 'woocommerce_thumbnail');
									if($image && !in_array($term->name, $arr_images)){
									...
									array_push($arr_images,$term->name);
									break;
								}
							}
						}		
