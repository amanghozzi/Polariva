...
class bwp_ajax_filter_widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		$show_brand		 = 	isset($instance['show_brand']) ? $instance['show_brand'] : 1;
		$layout_shop		 = 	isset( $instance['layout_shop'] ) ? $instance['layout_shop'] : 1;
		$show_price		 = 	isset($instance['show_price']) ? $instance['show_price'] : 1;
		$showcount		 = 	isset($instance['showcount']) ? $instance['showcount'] : 1;
		...
		echo '<div class="bwp-filter-price">';
			...
			echo '<div class="content-filter-price">';
				...
				if($min_price == $max_price){
					echo '<div id="bwp_slider_price" class="default_price" data-symbol="'.get_woocommerce_currency_symbol().'" data-min="'.$default_min_price.'" data-max="'.$default_max_price.'"></div>';
				}else{
									...
									if($count > 0 ){
										...
										if(isset($tax_attribute->attribute_type) && $tax_attribute->attribute_type == "color"){
											...
											echo '<li class="filter_image">';
													...
													echo apply_filters( 'woocommerce_layered_nav_count', '<label class="count">'.esc_html( $term->name ).''.esc_html($text_count).'<mark>'.esc_html($text_count2).'</mark></label>', $count, $term );
											echo	'</li> ';
										}
										else{
	...
	function woocommerce_filter_category($showcount){
		...
		if($terms){
			...
			foreach ($terms as $term){
				...
				echo '<div data-id_item="'.$term->term_id.'" class="item-taxonomy item-category '.( (count($terms_vl1) > 0) ?  'cat-parent' : '' ).' '.( ($option_is_set == 1) ?  'active' : '' ).'">';
					...
					if($showcount){ echo '<label class="count">('.($term->count).')</label>'; }
					echo '</a>';
					if($terms_vl1){
						echo '<div class="children">';
							foreach ($terms_vl1 as $term_vl){
								...
								echo '<div data-id_item="'.$term_vl->term_id.'" class="item-taxonomy item-category  '.( (count($terms_vl2) > 0) ?  'cat-parent' : '' ).' '.( ($option_is_set == 1) ?  'active' : '' ).'">';
									...
									if($showcount){ echo '<label class="count">('.($term_vl->count).')</label>'; }
									echo '</a>';
										if($terms_vl2){
											echo '<div class="children">';
												foreach ($terms_vl2 as $term_vl2){
													...
													echo '<div data-id_item="'.$term_vl2->term_id.'" class="item-taxonomy item-category '.( ($option_is_set == 1) ?  'active' : '' ).'">';
														...
														if($showcount){ echo '<label class="count">('.($term_vl2->count).')</label>'; }
														echo '</a>';
													echo '</div> ';
												}
											echo '</div>';
