...
function bwp_save_all_settings_via_ajax() {
	...
	if ( ! current_user_can( 'manage_options' ) ) {
		$response['message'][] = esc_html__( 'Cheating!? Huh?', 'polariva' );
		$response['err']       = 'yes';
		wp_send_json( $response );
	}
	
	if ( ! wp_verify_nonce( $nonce, 'bwp_backend_nonce' ) ) {
		$response['message'][] = esc_html__( 'Security check error!', 'polariva' );
		$response['err']       = 'yes';
		wp_send_json( $response );
	}
...
//	$response['new_all_settings'] = $new_all_settings;
	...
	$response['message'][] = esc_html__( 'All settings saved', 'polariva' );
	wp_send_json( $response );
	die();
}
