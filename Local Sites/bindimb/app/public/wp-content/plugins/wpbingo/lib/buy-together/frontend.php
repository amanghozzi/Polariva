...
if ( ! class_exists( 'bwpBuyTogetherFrontend' ) ) {
	class bwpBuyTogetherFrontend {
		...
		public function bwp_woo_product_tab( $tabs ) {
			...
			$default_tab_title = esc_html__( 'Frequently Bought Together', 'polariva' );
		...
		$bwp_ids = get_post_meta( $product_id, 'bwp_ids', true );
		if ( trim( $bwp_ids ) != '' ) {
			$add_to_cart_text = esc_html__( 'Add All To Cart', 'polariva' );
			$thumb_w = 300;
			$thumb_h = 300;
			...
			$bwp_checkboxs_list_html = '';
				...
				$bwp_checkboxs_list_html .= '<div data-product_id="' . esc_attr( $product_id ) . '" class="item-product buy-together-main-item ' . $avai_class . '">
										<label>
											...
											<span class="item-product-title"><strong>' . esc_html__( 'This product: ', 'polariva' ) . '</strong> ' . get_the_title( $product_id ) . '</span>
				...
				$total_price_html = '<div class="total-price-wrap">
										...
										<span class="for-items-text">' . sprintf( esc_html__( 'For %s item(s)', 'polariva' ), $total_items ) . '</span>
									</div>';
