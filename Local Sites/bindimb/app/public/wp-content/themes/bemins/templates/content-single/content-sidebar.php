	...
	<div class="container">
		<div class="content-image-single<?php if ( !get_the_post_thumbnail() ){ ?> no-thum<?php }; ?>">
			...
			<div class="content-info">
				...
				<?php endif; ?>	
				<?php
					$show_post_title = bemins_get_config('post-title',true);
					if ($show_post_title){
						if ( is_single() ){
		...
		<div class="single-post-content row">
			...
			<div class="bwp-sidebar sidebar-blog <?php echo esc_attr(bemins_get_class()->class_sidebar_left); ?>">
				<?php dynamic_sidebar('sidebar-blog');?>	
			</div>				
			<?php endif; ?>
			<div class="post-single <?php echo esc_attr($post_single_layout); ?> <?php echo esc_attr(bemins_get_class()->class_single_content); ?>">
				<article id="post-<?php esc_attr(the_ID()); ?>" <?php post_class(); ?>>
					...
					<div class="post-content-entry">
						...
						if ( 'post' === get_post_type() ) {
							$tags_list = get_the_tag_list( '', esc_html_x( '', 'list item separator', 'bemins' ) );
							if ( $tags_list ) {
								printf( '<div class="tags-links"><label>' . esc_html__( 'Tags :', 'bemins' ) . '</label> %1$s</div>', $tags_list ); // WPCS: XSS OK.
							}
						}
						?>
						<?php if ( shortcode_exists( 'social_share' ) ) : ?> 
							<div class="entry-social-share">
								<label><?php echo esc_html__('Share :','bemins'); ?></label>
