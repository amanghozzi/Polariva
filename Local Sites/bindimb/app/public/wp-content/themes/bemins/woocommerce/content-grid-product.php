...
<?php }elseif ($layout_shop == '5') { ?>
	...
	<div class="products-entry content-product5<?php if ( $product->is_type( 'variable' ) ){  ?> quick-shop<?php } ?> clearfix product-wapper" data-product_id="<?php echo esc_attr($product->get_id()); ?>">
		<div class="products-thumb">
			<?php if ( $product->is_type( 'variable' ) ){  ?>
				<?php 
					...
					if ( $attributes && $count_attr > 0 ) {
						echo '<div class="content-form-attribute"><div class="form-attribute"><div class="content"><div class="title disabled" data-title="'.esc_html__('SELECT OPTIONS','bemins').'" data-title_atc="'.esc_html__('ADD TO CART','bemins').'"></div>';
						foreach ( $attributes as $attribute ) {
							if($attribute['variation']){
								...
								if(isset($tax_attribute->attribute_type) && $tax_attribute->attribute_type != "color" && $tax_attribute->attribute_type != "image"){
									echo '<ul class="attribute-card option-'.$attribute['position'].'" data-position='.$attribute['position'].'>';
									if ( $attribute_values && ! is_wp_error( $attribute_values ) ) {
										foreach ( $attribute_values as $attribute_value ) {
											$term = get_term_by( 'id', $attribute_value->term_id, $attribute_name );
			...
			<div class='product-button'>
				<?php if ( $product->is_type( 'variable' ) ){  ?>
				<div class="form-variable variable-atc disabled" data-title="<?php echo esc_attr__('Select options','bemins') ?>" data-title_atc="<?php echo esc_attr__('Add to cart','bemins') ?>">
					<form class="cart" method="post" enctype="multipart/form-data" data-product_id="<?php echo esc_attr($product->get_id()); ?>">
						<button type="submit" class="single_add_to_cart_button button disabled"><span><?php echo esc_html__('Add to cart','bemins') ?></span></button>
		...
		<div class="products-content">
			<div class="contents">
				...
				<?php if ( $product->is_type( 'variable' ) ){  ?>
					<?php if ( $attributes ) {
						//echo'<pre>';print_r($attributes);die(1);
						...
						foreach ( $attributes as $attribute ) {
							if($attribute['variation']){
								...
								$attribute_values = get_the_terms( $product->get_id(), $attribute_name );
								if(isset($tax_attribute->attribute_type) && ($tax_attribute->attribute_type == "color" || $tax_attribute->attribute_type == "image")){
									echo '<ul class="attribute-card color option-'.$attribute['position'].'" data-position='.$attribute['position'].'>';
									if ( $attribute_values && ! is_wp_error( $attribute_values ) ) {
										foreach ( $attribute_values as $attribute_value ) {
											$term = get_term_by( 'id', $attribute_value->term_id, $attribute_name );
											if ( $term && $term->slug !== '' && $term->name !== '') {
					...
					<?php if ( $product->is_type( 'variable' ) ){  ?>
						<?php if ( $color_check != $variation_check ){  ?>
							<div class="gruop-btn-mobile">
								<div class="select-option-mobile <?php if($color_check > 0){ ?>disabled<?php } ?>"><?php echo esc_html__('Select options','bemins') ?></div>
								<div class="form-variable variable-atc disabled" data-title="<?php echo esc_attr('Select options','bemins') ?>" data-title_atc="<?php echo esc_attr('Add to cart','bemins') ?>">
									<form class="cart" method="post" enctype="multipart/form-data" data-product_id="<?php echo esc_attr($product->get_id()); ?>">
										<button type="submit" class="single_add_to_cart_button button disabled"><span><?php echo esc_html__('Add to cart','bemins') ?></span></button>
										<input type="hidden" name="add-to-cart" value="<?php echo esc_attr($product->get_id()); ?>">
										<input type="hidden" name="product_id" value="<?php echo esc_attr($product->get_id()); ?>">
						...
						<?php } ?>
					<?php }else{
						bemins_woocommerce_template_loop_add_to_cart();
					} ?>
				</div>
			</div>
		</div>
