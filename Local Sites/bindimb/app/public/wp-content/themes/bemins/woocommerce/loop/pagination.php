...
$shop_paging	= bemins_get_config('shop_paging','shop-pagination');
?>
...
<nav class="woocommerce-pagination <?php echo esc_attr($shop_paging); ?>">
	<div class="woocommerce-product-count">	
		...
		if ( 1 === $total ) {
			_e( 'Showing the single result', 'bemins' );
		} elseif ( $total <= $per_page || -1 === $per_page ) {
			printf( __( 'Showing all %d results', 'bemins' ), $total );
		} else {
			printf( _x( 'Showing %1$d&ndash;%2$d of %3$d item(s)', '%1$d = first, %2$d = last, %3$d = total', 'bemins' ), $first, $last, $total );
		}
		?>
	</div>
	...
	<div class="loadmore">
		<button type="button" class="woocommerce-load-more" data-paged="1">
			<span class="loadmore-button-text"><?php echo esc_html__('Load More', 'bemins'); ?></span>
