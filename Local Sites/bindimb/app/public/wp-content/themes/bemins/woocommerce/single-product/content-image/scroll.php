...
$video_style = bemins_get_config("video-style","inner");
...
$class = "";
if(bemins_image_single_product()->show_thumb =='show' && (bemins_image_single_product()->position == "left" || bemins_image_single_product()->position == "right"))
	$class = "vertical";
?>
<div class="images <?php echo esc_attr($class); ?>">
	<figure class="<?php echo esc_attr( implode( ' ', array_map( 'sanitize_html_class', $wrapper_classes ) ) ); ?>">
		<div class="row">
			<?php if(bemins_image_single_product()->show_thumb =='show' && bemins_image_single_product()->position == "left") : ?>
				<div class="<?php echo esc_attr(bemins_image_single_product()->class_thumb); ?>">
					<?php do_action( 'woocommerce_product_thumbnails' ); ?>
				</div>
			<?php endif; ?>
			<div class="<?php echo esc_attr(bemins_image_single_product()->class_image); ?>">
				<div class="container-image-single">
					<div class="scroll-image">
						<div class="image-additional slick-carousel" <?php if(bemins_image_single_product()->show_thumb == 'show') { ?> data-asnavfor=".image-thumbnail"<?php } ?> data-fade="true" data-columns4="1" data-columns3="1" data-columns2="1" data-columns1="1" data-columns="1" data-nav="true">
							<?php
							$attributes = array(
								'id'						=> "image", 	
								'title'                   => $image_title,
							...
							if ( has_post_thumbnail() ) {
								...
								$html .= sprintf( '<img src="%s" alt="%s" class="wp-post-image" />', esc_url( wc_placeholder_img_src() ), esc_html__( 'Awaiting product image', 'bemins' ) );
								$html .= '</div>';
							} 		
							...
							?>
								...
								<div class="video-additional text-center">
									<?php bemins_display_video_product($full_size_image); ?>
								</div>
							<?php } ?>
						</div>
					</div>
					<div class="function-button">
						<?php if($video_style == 'popup' && $video){ bemins_get_video_product(); } ?>
						<?php bemins_view_product(); ?>
					</div>
				</div>
			</div>
			<?php if(bemins_image_single_product()->show_thumb && (bemins_image_single_product()->position == "right" || bemins_image_single_product()->position == "bottom")) : ?>
				<div class="<?php echo esc_attr(bemins_image_single_product()->class_thumb); ?> content-thumbs-scroll">
				<?php do_action( 'woocommerce_product_thumbnails' ); ?>
				</div>
			<?php endif; ?>	
		</div>
